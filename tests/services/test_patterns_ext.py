...
class TestClassifyGlobFallback:
    def test_no_double_star_prefix(self) -> None:
        # Pattern without **/ prefix → GLOB fallback (line 40)
        m = _classify("foo/*.log")
        assert m.kind == _GLOB

    def test_double_star_dir_with_glob_chars(self) -> None:
        # **/segment/** where segment has glob chars → GLOB fallback (line 50)
        m = _classify("**/?foo/**")
        assert m.kind == _GLOB

    def test_rest_has_glob_chars(self) -> None:
        # **/rest where rest has glob chars (line 64)
